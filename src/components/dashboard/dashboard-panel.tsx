...
import type { DashboardPayload } from "@/features/transactions/types";
...
import { DashboardMonthlyExpenseChart } from "@/components/dashboard/dashboard-monthly-expense-chart";
import { transactionRailClass } from "@/lib/transactions/type-ui";

type LoansSummary = { youOwe: number; theyOweYou: number };
...
const statAccentClass: Record<StatAccent, string> = {
  ...
  receive: "!border-teal-500/25 bg-teal-500/[0.07] hover:!border-teal-400/35 hover:bg-teal-500/[0.1]",
  balance:
...
function SectionLabel({ children }: { children: ReactNode }) {
  return (
    <div className="mb-2 flex shrink-0 items-center gap-2">
      ...
      <span className="h-px flex-1 bg-linear-to-r from-primary/45 via-[var(--border)] to-transparent" />
    </div>
  );
}
...
function Stat({
  ...
  return (
    ...
    >
      <div className="pointer-events-none absolute -right-6 -top-6 h-20 w-20 rounded-full bg-white/[0.05] blur-2xl transition-opacity duration-500 group-hover:opacity-90" />
      <p className="relative text-[11px] font-semibold uppercase tracking-wide text-[var(--ink-muted)]">
        {label}
      </p>
      <p
        className={`relative mt-1 font-semibold tabular-nums tracking-tight text-[var(--ink)] ${largeValue ? "text-2xl" : "text-lg"}`}
      >
        {value}
      </p>
      {sub ? (
        <p className="relative mt-1.5 text-[10px] leading-relaxed text-[var(--ink-muted-2)]">
          {sub}
        </p>
      ) : null}
    </GlassCard>
...
function ActivityAmount({
  ...
  if (out) {
    return (
      <span className={`shrink-0 text-right text-rose-300 ${sz}`}>
        −{formatted}
      </span>
    );
  }
  return (
    <span className={`shrink-0 text-right text-emerald-300 ${sz}`}>
      +{formatted}
    </span>
  );
}

function LoansCard({ summary }: { summary: LoansSummary }) {
  const empty = summary.youOwe === 0 && summary.theyOweYou === 0;
  return (
    <GlassCard
      variant="signature"
      hideAccent
      ...
      panelClassName="flex flex-col gap-3 !p-3"
    >
      ...
      <div className="grid grid-cols-2 gap-2">
        <div className="rounded-xl border border-white/8 bg-white/[0.03] px-3 py-2">
          ...
          <p className="mt-1 text-lg font-semibold tabular-nums tracking-tight text-amber-200">
        ...
        <div className="rounded-xl border border-white/8 bg-white/[0.03] px-3 py-2">
          ...
          <p className="mt-1 text-lg font-semibold tabular-nums tracking-tight text-emerald-200">
            {formatCurrency(summary.theyOweYou)}
          </p>
        </div>
      </div>

      <p className="text-xs text-[var(--ink-muted)]">
        {empty ? "No active loan balances per contact." : "Tap contacts in Transactions for details."}
      </p>
    </GlassCard>
  );
}
...
export function DashboardPanel({ data }: { data: DashboardPayload }) {
  ...
  const trendHasData =
    ...
    monthlyExpenseTrend.some((d) => d.expense > 0);
  ...
  const peakActivityRow =
    recentActivity.length === 0
      ? null
      : recentActivity.reduce((best, r) =>
          Math.abs(r.amount) > Math.abs(best.amount) ? r : best
        );
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col gap-3 overflow-hidden">
      <PageHeader
        ...
        subtitle={
          <>
            <span className="font-medium text-[var(--ink)]">{monthLabel}</span>
            {" · "}
            trend and activity
          </>
        }
      ...
      <section className="grid min-h-0 flex-1 grid-cols-1 gap-3 overflow-x-hidden lg:grid-cols-2">
            ...
            <div className="flex flex-col items-stretch gap-2 min-[400px]:flex-row min-[400px]:items-start min-[400px]:justify-between min-[400px]:gap-3">
              <div className="flex min-w-0 min-h-0 flex-1 items-start gap-2">
                ...
                <div className="min-w-0 flex-1">
                  <h2 className="text-pretty text-sm font-semibold tracking-tight text-[var(--ink)]">
                    Monthly expense trend
                  </h2>
                  <p className="mt-0.5 text-pretty text-[11px] leading-snug text-[var(--ink-muted)]">
              ...
              {trendHasData ? (
                <div
                  className="grid w-full max-w-sm shrink-0 grid-rows-[auto_auto] gap-2 self-end rounded-lg border border-blue-500/25 bg-blue-500/[0.08] px-3 py-2 min-[400px]:w-[12.5rem] min-[400px]:max-w-[42%] min-[400px]:self-auto"
                  role="group"
                ...
                >
                  ...
                  <div className="flex items-end justify-between gap-2">
                    <span className="min-w-0 truncate text-sm font-semibold tabular-nums leading-none text-[var(--ink)]">
                      {formatCurrency(peakMonth.expense)}
                    </span>
                    <span className="shrink-0 text-right text-[10px] leading-snug text-[var(--ink-muted)]">
        ...
        <div className="flex min-h-0 flex-col gap-3">
          <LoansCard summary={loansSummary} />
          ...
          >
                ...
                {peakActivityRow ? (
                  ...
                  >
                    ...
                    <div className="flex items-end justify-between gap-2">
                      <ActivityAmount
                        compact
                        type={peakActivityRow.type}
                        amount={peakActivityRow.amount}
                      />
                      <span className="max-w-[55%] truncate text-right text-[10px] leading-snug text-[var(--ink-muted)]">
            ...
            {recentActivity.length === 0 ? (
              <p className="rounded-2xl border border-dashed border-[var(--border)] bg-[var(--glass-simple-bg)]/50 py-8 text-center text-sm text-[var(--ink-muted)]">
                No transactions yet.
              </p>
            ) : (
              <ul className="scrollbar-hide flex min-h-0 flex-1 flex-col gap-1.5 overflow-y-auto overscroll-contain rounded-lg bg-[var(--glass-simple-bg)] shadow-[inset_0_1px_0_rgba(255,255,255,0.04)]">
                {recentActivity.map((row) => {
                  ...
                  return (
                    <li key={row.id}>
                      <div className="group/row relative flex gap-2 overflow-hidden rounded-lg border border-white/[0.08] bg-white/[0.03] px-2.5 py-2 transition-all duration-200 hover:border-blue-500/25 hover:bg-white/[0.05] hover:shadow-[0_0_0_1px_rgba(59,130,246,0.12)]">
                        <div
                        ...
                        <div className="min-w-0 flex-1">
                          <div className="flex items-baseline justify-between gap-2">
                            <p className="min-w-0 truncate text-xs font-medium leading-snug tracking-tight text-[var(--ink)]">
                              {row.title}
                            </p>
                            <ActivityAmount
                              compact
                          ...
                          <TransactionTypeChip type={row.type} />
                            <span className="min-w-0 truncate text-[9px] font-medium uppercase tracking-wide text-[var(--ink-muted-2)]">
                              {loc}
                            </span>
                          </div>
                        </div>
