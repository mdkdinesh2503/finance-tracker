...
export function CategoryCascadeFields({
  ...
  const subOptions = useMemo(
    () => subs.map((c) => ({ id: c.id, name: c.name })),
    [subs]
  );

  const subEnabled =
    Boolean(parentId) && subs.length > 0 && !disabled;
      ...
      <div className="grid gap-4 sm:grid-cols-2 sm:gap-5">
        ...
        <div className="space-y-2">
          <Label
            ...
            className={!parentId ? "text-zinc-500" : undefined}
          >
            Subcategory
          </Label>
          <DropdownSelect
...
/** Derive parent id from a leaf category id for initial state. */
