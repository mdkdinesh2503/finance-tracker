    ...
    <div className="space-y-8">
      <div className="grid gap-4 md:grid-cols-3">
        {Array.from({ length: 3 }).map((_, i) => (
          <div
            key={i}
            className="relative overflow-hidden rounded-2xl border border-white/15 bg-linear-to-br from-white/[0.07] to-surface p-6 shadow-(--shadow-card)"
          >
            <Skeleton className="h-2.5 w-24 rounded-full" />
            <Skeleton className="mt-4 h-10 w-36 sm:h-12" />
            <Skeleton className="mt-3 h-3 w-full max-w-56" />
          </div>
        ))}
      </div>
