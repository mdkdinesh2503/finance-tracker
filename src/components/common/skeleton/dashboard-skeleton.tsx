import { PageHeaderSkeleton } from "@/components/common/skeleton/page-header-skeleton";
...
function SectionLabelSkeleton() {
  return (
    <div className="mb-2 flex shrink-0 items-center gap-2">
      ...
      <span className="h-px flex-1 bg-linear-to-r from-primary/30 via-(--border) to-transparent" />
    </div>
  );
}
      ...
      <section className="grid min-h-0 flex-1 grid-cols-1 gap-3 overflow-x-hidden lg:grid-cols-2">
        ...
        >
          <div className="mb-2 flex flex-col items-stretch gap-2 min-[400px]:flex-row min-[400px]:items-start min-[400px]:justify-between min-[400px]:gap-3">
            ...
            <Skeleton className="h-13 w-full max-w-sm shrink-0 self-end rounded-lg min-[400px]:w-50 min-[400px]:max-w-[42%] min-[400px]:self-auto" />
          </div>
        ...
        >
          ...
          <ul className="scrollbar-hide flex min-h-0 flex-1 flex-col gap-1.5 overflow-y-auto overscroll-contain rounded-lg bg-(--glass-simple-bg) p-1.5 shadow-[inset_0_1px_0_rgba(255,255,255,0.04)]">
            {Array.from({ length: 5 }).map((_, i) => (
              <li
                key={i}
                className="flex gap-2 rounded-lg border border-white/8 bg-white/3 px-2.5 py-2"
              >
