...
function computeSalaryRaiseInsight(
  ...
  const pct = first.total > 0 ? (growth / first.total) * 100 : null;
...
async function loadLifetimeIncomeAnalytics(db: Db, userId: string) {
  ...
  let salaryWagesTotal = 0;
  let otherIncomeTotal = 0;
  const lifetimeByParent = (byParentRows as unknown as { parent_name: string; total: string }[])
    .map((r) => {
      const t = num(r.total);
      totalIncome += t;
  ...
  const lifetimeByLeaf = (leafRows as unknown as { parent_name: string; leaf_name: string; total: string }[])
    .map((r) => ({
  ...
  const employerRows = (employerRaw as unknown as { company_name: string; total: string }[])
    .map((r) => ({
  ...
  return {
    ...
    lifetimeByParent,
    lifetimeByLeaf,
    lifetimeFamilySupportTotal: num((famRow as { total: string } | undefined)?.total),
...
async function fetchSalaryEmployerMonthlyCells(
  ...
  return (rows as unknown as { ym: string; company_name: string; total: string }[]).map((r) => ({
    ym: r.ym,
    companyName: r.company_name ?? "Unspecified",
    total: num(r.total),
  }));
...
export async function incomeAnalyticsSnapshot(
  ...
  const byParentThisMonth = (byParentRaw as unknown as { parent_name: string; total: string }[])
    .map((r) => ({
      parentName: r.parent_name ?? "Uncategorized",
      total: num(r.total),
    }))
  ...
  const byLeafThisMonth = (byLeafRaw as unknown as { parent_name: string; leaf_name: string; total: string }[])
    .map((r) => ({
      parentName: r.parent_name ?? "Uncategorized",
      leafName: r.leaf_name ?? "Uncategorized",
      total: num(r.total),
  ...
  const salaryWagesMonthly = (salaryRows as unknown as { ym: string; total: string }[]).map((r) => ({
    ym: r.ym,
    total: num(r.total),
  }));
  ...
  const otherIncomeMonthly = (otherIncomeRows as unknown as { ym: string; total: string }[]).map((r) => ({
    ym: r.ym,
    total: num(r.total),
  }));
  ...
  for (const line of salaryTxLines as unknown as { transaction_date: string; amount: string }[]) {
  ...
  if (scope === "salary") {
    ...
    const employerSalaryInsights = buildEmployerSalaryInsights(salaryEmployerMonthlyCells);
    ...
    return {
      ...
      byParentThisMonth: byParentThisMonth.filter((r) => r.parentName === SALARY_WAGES_PARENT),
      byLeafThisMonth: byLeafThisMonth.filter((r) => r.parentName === SALARY_WAGES_PARENT),
      salaryWagesMonthly,
      ...
      lifetimeByParent: lifetime.lifetimeByParent.filter((r) => r.parentName === SALARY_WAGES_PARENT),
      lifetimeByLeaf: lifetime.lifetimeByLeaf.filter((r) => r.parentName === SALARY_WAGES_PARENT),
  ...
  return {
    ...
    byParentThisMonth: byParentThisMonth.filter((r) => r.parentName === OTHER_INCOME_PARENT),
    byLeafThisMonth: byLeafThisMonth.filter((r) => r.parentName === OTHER_INCOME_PARENT),
    salaryWagesMonthly: [],
    salaryWagesSpendAlignedMonthly: [],
    otherIncomeMonthly,
    ...
    lifetimeByParent: lifetime.lifetimeByParent.filter((r) => r.parentName === OTHER_INCOME_PARENT),
    lifetimeByLeaf: lifetime.lifetimeByLeaf.filter((r) => r.parentName === OTHER_INCOME_PARENT),
